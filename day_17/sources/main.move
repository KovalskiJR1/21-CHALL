module challenge::farm_simulator {
    ...
    fun test_create_farm_entry() {
        ...
        // Create farm using entry function
        {
            let ctx = test_scenario::ctx(scenario);
            create_farm(ctx);
        };
        
        // Verify farm was created and shared
    ...
    #[test]
    fun test_multiple_users_shared_farm() {
        let owner = @0xA;
        let user1 = @0xB;
        let user2 = @0xC;
        let mut scenario_val = test_scenario::begin(owner);
    ...
    fun test_plant_on_farm_invalid_plotid() {
        let owner = @0xA;
        let mut scenario_val = test_scenario::begin(owner);
        let scenario = &mut scenario_val;
        ...
        {
            let mut farm = test_scenario::take_shared<Farm>(scenario);
            plant_on_farm(&mut farm, 0); // Invalid plotId
            test_scenario::return_shared(farm);
        };
    ...
    fun test_plant_on_farm_duplicate() {
        let owner = @0xA;
        let mut scenario_val = test_scenario::begin(owner);
        let scenario = &mut scenario_val;
        ...
        test_scenario::next_tx(scenario, owner);
        {
            let mut farm = test_scenario::take_shared<Farm>(scenario);
            plant_on_farm(&mut farm, 1);
            plant_on_farm(&mut farm, 1); // Duplicate
            test_scenario::return_shared(farm);
        };
    ...
    fun test_harvest_from_farm_unplanted() {
        let owner = @0xA;
        let mut scenario_val = test_scenario::begin(owner);
        let scenario = &mut scenario_val;
        ...
        {
            let mut farm = test_scenario::take_shared<Farm>(scenario);
            harvest_from_farm(&mut farm, 1); // Not planted
            test_scenario::return_shared(farm);
        };
